...
class TextToSpeechService:
    def __init__(self, device: str = "cuda" if torch.cuda.is_available() else "cpu"):
        ...
        self.fp16 = torch.cuda.is_available()  # Enable mixed precision if GPU is available
    ...
    def synthesize(self, text: str, voice_preset: str = "v2/en_speaker_1"):
        ...
        with torch.no_grad(), torch.amp.autocast(self.device, enabled=self.fp16):  # Updated autocast syntax
    ...
    def long_form_synthesize(self, text: str, voice_preset: str = "v2/en_speaker_1"):
        ...
        sample_rate = self.model.generation_config.sample_rate
        silence = torch.zeros(int(0.25 * sample_rate), dtype=torch.float32, device=self.device)

        audio_pieces = []
        for sent in sentences:
            _, audio_array = self.synthesize(sent, voice_preset)
            audio_tensor = torch.tensor(audio_array, device=self.device)
            audio_pieces.append(audio_tensor)
            audio_pieces.append(silence)

        # Concatenate audio pieces on GPU and transfer back to CPU at the end
        final_audio = torch.cat(audio_pieces).cpu().numpy()
